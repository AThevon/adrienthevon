...
export default function ProjectTakeover({
  ...
  return (
    <div className="px-6 md:px-12 py-8 space-y-8 max-w-7xl mx-auto">
      ...
      {/* Preview: iframe for sites, compact card for GitHub-only */}
      {!isGithub ? (
        <div
...
/** Compact GitHub-only preview (no iframe) */
function GithubPreview({ project }: { project: ReturnType<typeof getProjectById> }) {
  if (!project) return null;
  ...
  return (
    <div className="border border-foreground/10 p-8 md:p-12 flex items-center gap-8">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="font-mono text-sm text-foreground/70 leading-relaxed mb-4">
          {project.description}
        </div>
